...
type var_declaration =
    CDECL of locator * string (* déclaration de variable de type int. *)
...
let setop_text setop = match setop with
    S_MUL -> "*="
  | S_DIV -> "/="
  | S_MOD -> "%="
  | S_ADD -> "+="
...
let mop_text mop = match mop with
    M_MINUS -> "-"
  | M_NOT -> "~"
  | M_POST_INC | M_PRE_INC -> "++"
  | M_POST_DEC | M_PRE_DEC -> "--"
                            
let mop_prec mop = match mop with
    M_MINUS -> uminus_prec
  | M_NOT -> tilde_prec
  | M_POST_INC | M_POST_DEC | M_PRE_INC | M_PRE_DEC -> incdec_prec
                                                     
let op_text setop = match setop with
    S_MUL -> "*"
  | S_DIV -> "/"
  | S_MOD -> "%"
  | S_ADD -> "+"
...
let fin_op_text setop = match setop with
    S_MUL -> ""
  | S_DIV -> ""
  | S_MOD -> ""
  | S_ADD -> ""
...
let op_prec setop = match setop with
    S_MUL -> mul_prec
  | S_DIV -> div_prec
  | S_MOD -> mod_prec
  | S_ADD -> add_prec
...
let rec bufout_expr buf pri e = match e with
...
and bufout_loc_expr_list buf l = match l with
    [] -> ()
